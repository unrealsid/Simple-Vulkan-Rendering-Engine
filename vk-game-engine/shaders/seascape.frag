...
#define EPSILON_NRM (0.1 / globalData.resolution.x)
#define AA
...
#define SEA_TIME (1.0 + globalData.time.x * SEA_SPEED)
...
vec3 getPixel(in vec2 coord, float time) {    
    vec2 uv = coord / globalData.resolution.xy;
    uv = uv * 2.0 - 1.0;
    uv.x *= globalData.resolution.x / globalData.resolution.y;
    uv.y *= -1.0;
...
// main
void main( ) {
    float time = globalData.time.x * 0.3 + 1.0*0.01;
