...
float noise( in vec3 x )
{
	...
	vec2 rg = textureLod( sampler2D(textures[0], samplerObj), (uv+ 0.5)/256.0, 0.0 ).yx;
...
{
	#ifdef ROTATION
	R(p.xz, 1.0*0.008*pi+globalData.time.x*0.1);
	#endif
...
{  
	...
	vec3 rd = normalize(vec3((gl_FragCoord.xy-0.5*globalData.resolution.xy)/globalData.resolution.y, 1.));
    ...
    #ifdef DITHERING
	vec2 dpos = ( gl_FragCoord.xy / globalData.resolution.xy );
	vec2 seed = dpos + fract(globalData.time.x);
	#endif 
    ...
    #ifdef TONEMAPPING
    outFragColor = vec4(ToneMapFilmicALU(sum.xyz*2.2),1.0);
	#else
